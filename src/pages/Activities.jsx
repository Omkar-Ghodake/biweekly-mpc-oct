      ...
      <div className='flex items-center space-x-5 w-full h-[75vh]'>
        ...
        <ActivityDetails>
          <div className='activity-head flex flex-col space-y-2 h-fit'>
            <ActivityTitle>
              {ACTIVITIES_DATA[selectedActivity].title}
            </ActivityTitle>

            {ACTIVITIES_DATA[selectedActivity].activityInfo && (
              <ActivityInfo>
                {ACTIVITIES_DATA[selectedActivity].activityInfo}
              </ActivityInfo>
            )}
          ...
          <div className='activity-body h-[50vh] flex space-x-5 text-xl'>
            <>
              <div className='left w-1/2 border border-sky-800 rounded-md flex justify-center items-center'>
                {ACTIVITIES_DATA[selectedActivity].chartComponent}
              </div>

              <div className='right w-1/2 p-5 border border-sky-800 rounded-md flex flex-col'>
                <div className='flex flex-col space-y-5'>
                  {ACTIVITIES_DATA[selectedActivity].otherData.length > 0 &&
                    ACTIVITIES_DATA[selectedActivity].otherData.map((obj) => {
                      return (
                        <>
                          <div className='flex'>
                  ...
                  {ACTIVITIES_DATA[selectedActivity].members.length > 0 &&
                    ACTIVITIES_DATA[selectedActivity].members.map((obj) => {
                      return (
                        <div className='flex'>
                          <p className='text-orange-400 w-2/5'>{obj.key}</p>
